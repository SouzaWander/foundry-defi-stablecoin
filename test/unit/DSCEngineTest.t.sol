...
contract DSCEngineTest is Test {
    ...
    function testRevertsIfMintedDscBreaksHealthFactor() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(DSCEngine.DSCEngine__BreacksHealthFactor.selector, expectedHealthFactor));
        dsce.depositCollateralAndMintDSC(weth, AMOUNT_COLLATERAL, amountToMint);
        vm.stopPrank();
    }
    ...
    function testRvertsIfMintAmountBreaksHealthFacto() public depositCollateral {
        (, int256 price,,,) = MockV3Aggregator(ethUsdPriceFeed).latestRoundData();
        amountToMint = (AMOUNT_COLLATERAL * (uint256(price) * dsce.getAdditionalFeedPrecision())) / dsce.getPrecision();
    ...
    ///////////////////////////
    function testRevertsIfRedeemForDSCAmountIsZero() public depositedCollateralAndMintedDsc {
        vm.startPrank(USER);
        dsc.approve(address(dsce), amountToMint);
    ...
    function testRedeemDepositedCollateral() public {
        vm.startPrank(USER);
        ERC20Mock(weth).approve(address(dsce), AMOUNT_COLLATERAL);
        dsce.depositCollateralAndMintDSC(weth, AMOUNT_COLLATERAL, amountToMint);
        dsc.approve(address(dsce), amountToMint);
